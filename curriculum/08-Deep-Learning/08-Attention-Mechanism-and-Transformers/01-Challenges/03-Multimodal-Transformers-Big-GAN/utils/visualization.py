...
def text_to_image(text, mapping_model,
    ...
    tokens = lm_tokenizer.encode(text, add_special_tokens=True, return_tensors='tf')
    ...
    #
    mapping_output = torch.Tensor(mapping_model(lm_hidden_states_first_example).numpy()[:1])
    #
    ####################################
    ...
    numpy_image = generate_image(mapping_output,
                                 ...
                                 noise_seed_vector=torch.Tensor(tokens.numpy()) if noise_seed is None else noise_seed)
